...
export default function Menu(props) {
  ...
  const menuItem = (child) => {
    return child.map((v) => (
      <Pressable
        android_ripple={{ color: "rgba(0, 0,0,.2)", foreground: true }}
        onPress={() => changeRoute(v.name)}
        key={v.name}
        className="flex-col items-center w-1/3 mb-6 p-2 rounded-md overflow-hidden"
      >
        <Image
        ...
        />
        <Text className="mt-3 text-gray-900 font-medium text-sm text-center">
          {translate(v.name)}
        </Text>
      </Pressable>
    ));
    ...
    <View>
      {MenuData.length === 0 ? (
        <View>
          ...
          <View className="flex-row  mt-4 mb-5 flex-wrap">{skeleton()}</View>
        </View>
      ) : (
